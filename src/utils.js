...
export function removeClass(element, value) {
  ...
  if (element.className.indexOf(value) >= 0) {
    ...
    element.className = element.className.replaceAll(reg, '');
  }
}
...
export function closestByClass(element, clazz) {
  const reg = new RegExp(`${clazz}(\\s|$)`);

  while (!reg.test(element.className)) {
